...
**まだ完成していません。** 11周目の時点で15本中1本しか「可」になっていない。
残りを「可」にするのがこの引き継ぎの目的です。
...
- **「size だけあって中身（font_px / stroke_px）が無い」role が16件。** これが最大の塊。
  埋めると必ず派生値の連鎖が起きる（1つ埋めたら5roleの座標がやり直しになった実例あり）
- 15本中「可」は `documentary_cinematic` のみ（R10時点。その後さらに修正が入っている）
- **修正した直後は必ず未検証**。11周のうち5回、直した直後に別の破綻を作っている
