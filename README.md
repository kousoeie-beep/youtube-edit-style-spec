...
**[TASK_R12_remeasure.md](TASK_R12_remeasure.md) — pixel_collision に依存した判断の測り直し。**

`overlap_qc.py` にアルファ切り出しのオフセット漏れがあり、2026-07-31 に修正しました。
それまでの11周は狂った `pixel_collision` を根拠に「実害あり／なし」を判断しています。
bbox（矩形の交差）は正しいので大半の結論は変わらないはずですが、
**bbox比 1〜5% の帯域は pixel だけで拾う／捨てるを決めている**ので保証がありません。

作業指示は上記ファイルに、前提知識なしで実行できる形で書いてあります。
